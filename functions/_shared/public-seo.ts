...
type ImageMeta = {width:number;height:number;mime:string;alt:string};
type PublicSeo = {title:string; description:string; url:string; image:string; imageMeta?:ImageMeta; type:string; schema:Record<string, unknown>; body:string; article?:{published:string;modified:string;section:string;tags:string[]}};
...
const parseImageMeta = (value: unknown, fallbackAlt:string): ImageMeta | undefined => { try { const meta=JSON.parse(String(value ?? 'null')); return meta && Number.isInteger(meta.width) && Number.isInteger(meta.height) && ['image/png','image/jpeg','image/webp'].includes(meta.mime) ? {width:meta.width,height:meta.height,mime:meta.mime,alt:String(meta.alt || fallbackAlt)} : undefined; } catch { return undefined; } };
const renderPublicSeo = (html:string, {title,description,url,image,imageMeta,type,schema,body,article}:PublicSeo) => {
  ...
  if (imageMeta) meta.push(['property','og:image:width',String(imageMeta.width)],['property','og:image:height',String(imageMeta.height)],['property','og:image:type',imageMeta.mime]);
...
export const renderPublishedPostSeo = (html:string, row:Record<string, unknown>) => {
  ...
  const imageMeta = parseImageMeta(row.image_meta_json, post.title);
  ...
  const schema = {'@context':'https://schema.org','@type':'BlogPosting','@id':`${url}#article`,headline:post.title,description,url,mainEntityOfPage:{'@type':'WebPage','@id':url},image:{'@type':'ImageObject',url:image,...(imageMeta ? {width:imageMeta.width,height:imageMeta.height,encodingFormat:imageMeta.mime} : {})},datePublished:published,dateModified:modified,inLanguage:'pt-BR',articleSection:section,keywords:tags.map(tag=>tag.name).join(', '),author:{'@type':'Organization','@id':organizationId,name:'Código5',url:site},publisher:{'@type':'Organization','@id':organizationId,name:'Código5',url:site,logo:{'@type':'ImageObject',url:`${site}/assets/codigo5/logos/logo-dark.webp`}}};
  const dimensions=imageMeta ? ` width="${imageMeta.width}" height="${imageMeta.height}"` : '';
  const body=`<div class="c5-site"><header class="c5-container"><a href="/">Código5</a> · <a href="/blog">Blog</a></header><main id="conteudo" class="c5-container c5-prose"><article><h1>${escape(post.title)}</h1><p>${escape(post.excerpt)}</p><img class="c5-article-cover" src="${escape(image)}" alt="${escape(imageMeta?.alt || post.title)}"${dimensions} /><div style="white-space:pre-line">${escape(textOnly(post.contentHtml))}</div></article></main></div>`;
  return renderPublicSeo(html,{title,description,url,image,imageMeta,type:'article',schema,body,article:{published,modified,section,tags:tags.map(tag=>tag.name)}});
};
