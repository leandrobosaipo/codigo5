...
it('renders fresh editorial title, image and safe content without client JavaScript',()=>{
 ...
 expect(html).not.toContain('onerror');expect(html).not.toContain('alert(1)');expect(html).toContain('BlogPosting');expect(html).toContain('ImageObject');expect(html).toContain('og:image:secure_url');expect(html).toContain('og:image:width');expect(html).toContain('og:image:type');expect(html).toContain('width="1536" height="1024"');expect(html).toContain('article:published_time');expect(html).toContain('twitter:card');expect(html.match(/id="codigo5-schema"/g)).toHaveLength(1);
});
